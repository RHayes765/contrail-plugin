...
Read this when a generated `ReportType` component is rejected by
`validate_deploy` or fields don't appear in the report builder. Use these to
validate the authored XML before proposing the deploy.

| Error | Cause | Fix |
|-------|-------|-----|
| `Invalid object name 'X'` on `<baseObject>` | Primary object doesn't exist or isn't deployed | Deploy the custom object before the CRT (or in the same package) |
| `Invalid relationship name 'X'` on `<join>` | Used the field API name instead of the child relationship name, or forgot `__r` | Use the child relationship name (e.g. `Projects__r` for a custom relationship) — `describe_schema` on the parent lists them |
| `Invalid field 'X' for object 'Y'` | Field doesn't exist on `<table>`, used label instead of API name, or field not yet deployed | Verify the field API name via `describe_schema`; deploy dependent fields first |
...
| Component not found / fullName mismatch | The component `api_name` doesn't match the document's `<fullName>` | Make `<fullName>` and `api_name` the identical string (same casing and underscores) |
