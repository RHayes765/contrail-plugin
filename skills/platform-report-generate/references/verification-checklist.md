...
Run through these checks before calling `validate_deploy` on any generated
report metadata.

## Universal Checks
- [ ] Component shape: `{type: "Report", api_name: "FolderDevName/ReportDevName", content: full document}` — XML declaration + `<Report xmlns="http://soap.sforce.com/2006/04/metadata">` root
- [ ] The api_name's folder prefix names a folder that exists in the org, or a `ReportFolder` component (with `<folderShares>`) ships in the same package (`unfiled$public` for unfiled reports)
...
- [ ] `<booleanFilter>` references all criteria items by number if used
