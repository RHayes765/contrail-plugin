...
These names are report-type-specific — ALWAYS ground them against the org:
`retrieve_metadata` an existing report of the same report type (its `<columns>`
show the real platform column names) and `describe_schema` to confirm the
underlying fields exist. The tables below are the static mapping for common
...
Column names for reports built on a custom report type are NOT reliably
guessable from these tables. Do not invent them: `retrieve_metadata` an
existing report built on that CRT and copy the `<columns>` names it uses — or
build one throwaway report in the org's report builder UI, retrieve it, and
read the names from there.
